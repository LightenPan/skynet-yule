...
local function send_pbrequest()
	...
	data.head_flag = 1
	data.client_ver = 13
	local bytehead = protobuf.encode("app_protos.CSHead", data)
...
while true do
	...
	if cmd then
		...
		-- elseif cmd == "query" then
			-- send_request("query", { pbcmd = 101, pbsubcmd = 4 })
			-- send_request("query", { pbcmd = 101, pbsubcmd = 1 })
			-- send_request("query", { pbcmd = 100 })
		elseif cmd == "pb" then
			send_pbrequest("query")
		-- else
